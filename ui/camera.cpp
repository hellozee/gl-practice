...
Camera::Camera(glm::vec3 cameraPos, glm::vec3 cameraFront, glm::vec3 cameraUp,
               GLfloat fov, GLfloat ratio, GLfloat nearPlane,
               GLfloat farPlane) :
    cameraPos(cameraPos), _cameraFront(cameraFront), _cameraUp(cameraUp), _keys(
        322, false)
{
    _projection = glm::perspective(glm::radians(
                                       fov), ratio, nearPlane, farPlane);
    _cameraSpeed = 0.05f;
    _deltaTime   = 0;
    _lastFrame   = 0;
    _pitch       = 0.0f;
...
void Camera::doMovement(unsigned int currentFrame)
{
    ...
    if (_keys[SDLK_a]) {
        cameraPos -=
            glm::normalize(glm::cross(_cameraFront, _cameraUp)) * _cameraSpeed;
    }
